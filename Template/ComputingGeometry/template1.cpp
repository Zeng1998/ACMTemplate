...
double RC_Triangle(Point* res,int n){
    ...
    for(int i=1;i<=n;i++){
        ...
        k = (j+1)%n;
        while((j != k) && (k != i)){
            while(Cross(res[j] - res[i], res[k+1] - res[i]) > Cross(res[j] - res[i], res[k] - res[i])){
                k= (k+1)%n;
            }
    ...
    for(int i=1;i<=n;i++){
        ...
        if (cnt == 1){
            if (InCircle(p[i], C) && !InCircle(p[i+1], C)){
                ret+=Cross(sol[0]-C.c,p[i]-C.c)/2.0, ret+=SegCircleArea(C,sol[0],p[i+1]);
            }else{
                ret+=SegCircleArea(C,p[i],sol[0]), ret+=Cross(p[i+1]-C.c,sol[0]-C.c)/2.0;
            }
        }
        if (cnt == 2){
